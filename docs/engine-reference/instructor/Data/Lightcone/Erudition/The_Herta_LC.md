...
|---|---|
| CR `10 + 2S` | `Reset_List` |
| ใช้ Ult → Skill/Ult DMG `50 + 10S` 3 เทิร์น | `AllyActionList` → `act->isSameAction(ptr, AType::Ult)` |
...
ใช้ `AllyActionList` (เห็นทั้ง attack และ buff action) แทน `WhenUseUlt_List` — ต่างจาก LC ส่วนใหญ่ · ได้ผลเหมือนกันสำหรับ ult ที่เป็น action แต่ `WhenUseUlt_List` ตรงกว่า
