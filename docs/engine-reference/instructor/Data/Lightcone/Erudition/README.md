...
| `Rappa_LC.h` | `Rappa_LC` | `Rappa_LC` | BE `50+10S` | ต้นเกม energy `27.5+2.5S` · หลัง ult ใช้ BA 2 ครั้ง → advance `45+5S` |
...
| `Calculus.h` | `Calculus` | `Calculus` | ATK% `7+S` | หลังโจมตี ATK `min(เป้า,5)×(3+S)` ถึงครั้งถัดไป · โดน ≥ 3 เป้า → SPD `6+2S` |
...
| `BP_Erudition.h` (Today Is Another Peaceful Day) | `BP_Erudition` | `BP_Erudition` | DMG ตาม `Max_energy` | — |
...
buffNote["Calculus_Atk_buff"] = min(act->targetList.size(), 5) * (3 + superimpose);
ptr->Stats_type[ATK_P][None] += buffNote["Calculus_Atk_buff"];
```
