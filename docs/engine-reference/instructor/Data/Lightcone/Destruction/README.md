...
| `Mydei_LC.h` | `Mydei_LC` | `Mydei_LC` | HP% `15+3S` · Healing Out `15+5S` | Skill/Ult → DMG `25+5S` (×2 ถ้า HP พอจ่าย) เฉพาะ action นั้น |
| `Saber_LC.h` | `Saber_LC` | `Saber_LC` | CD `27+9S` | กด ult → ATK `30+10S` 2 เทิร์น (×2 + energy ถ้า `Max_energy >= 300`) |
| `FireFly_LC.h` | `FireFly_LC` | `FireFly_LC` | BE `50+10S` | โจมตี → เป้าติด Break VUL `20+4S` + SPD −20 |
...
| `Clara_LC.h` | `Clara_LC` | `Clara_LC` | ATK% `20+4S` | ถูกตี → ฮีลตัวเอง + DMG `20+4S` 1 เทิร์น |
| `Danheng_LC.h` | `Danheng_LC` | `Danheng_LC` | CR `15+3S` | ใช้ BA → ATK + ER stack (cap 2) 2 เทิร์น |
| `HertaShop.h` | `Hertashop` | `Fall of an Aeon` | — | โจมตี → ATK stack (cap 4) · มีการ break → DMG `9+3S` 2 เทิร์น |
...
- **`FireFly_LC.h` guard ผู้โจมตีด้วยเงื่อนไขที่เขียนกลับด้าน** — ดู `FireFly_LC.md`
- **`Mydei_LC.h` เทียบ `currentHP` กับค่าคงที่ `50000.0 / (5.5 + 0.5S)`** ซึ่งเป็นตัวเลขที่ไม่ได้อิงกับ Max HP จริง
