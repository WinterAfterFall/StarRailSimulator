...
| `BlackSwan_LC.h` | `BlackSwan_LC` | `BlackSwan_LC` | EHR `35+5S` | ศัตรูติด DoT ชนิดไหน → ATK/DEF_SHRED เพิ่มชนิดละครั้ง (4 ชนิด) |
| `Hysilens_LC.h` | `Hysilens_LC` | `Hysilens_LC` | EHR `35+5S` | ทีม SPD `7.5+2.5S` · ลง debuff → VUL[Dot] stack ตามจำนวน debuff ที่เพิ่ม (cap 6) |
| `Cipher_LC.h` | `Cipher_LC` | `Cipher_LC` | SPD% `15+3S` | ก่อนโจมตี → ศัตรูทุกตัวติด DEF_SHRED 2 ชั้น (`Bamboozle` + `Theft`) |
| `Fugue_LC.h` | `Fugue_LC` | `Fugue_LC` | BE `50+10S` | มีการ break → เป้าติด VUL `15+3S` 2 เทิร์น |
| `Jiaoqiu_LC.h` | `Jiaoqiu_LC(S, **bool isDot**)` | `Jiaoqiu_LC` | EHR `40+10S` | BA/Skill/Ult → เป้าติด VUL · ค่าต่างกันตาม `isDot` |
...
| `Before_the_Tutorial_Mission_Starts.h` | `Before_the_Tutorial` | `Before_the_Tutorial` | EHR `20+5S` | โจมตีเป้าที่ DEF ถูกลด → energy `3+S` |
...
`TriggerBySomeAlly_Func` รับ `(Enemy *target, AllyUnit *Trigger)` — ยิงก่อน/หลังการลง debuff · ใช้โดย `BlackSwan_LC.h`, `Hysilens_LC.h`, `ShowTime.h`
...
- **`Cipher_LC.h` ลง debuff ใน `BeforeAttackAction_List` โดยไม่ guard ผู้โจมตี** → ลงทุกครั้งที่ใครก็ตามโจมตี
