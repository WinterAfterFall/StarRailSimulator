...
|---|---|---|---|
| `Cerydra LC.h` | signature | ATK% `48+16S` | Skill เป้าเดียว → เป้าได้ Skill DMG `40.5+13.5S` 3 เทิร์น · ult โจมตี → คืน SP 1 |
...
| `Bronya_LC.h` | signature | ER `8+2S` | ใช้ Skill → เพื่อนที่เล่นถัดไป (ยกเว้นผู้สวม) ได้ DMG `25+5S` · กด ult สลับครั้งเว้นครั้งคืน SP 1 |
| `DDD.h` | 4★ | — | **กด ult → advance ทั้งทีม `14+2S`%** |
| `Memories_of_the_Past.h` | 4★ | BE `21+7S` | โจมตี → energy `3+S` (ครั้งเดียวต่อเทิร์น) |
| `The Forever Victual.h` | free | ATK% `12+4S` | ใช้ Skill → ATK stack `6+2S` (cap 3, ถาวรตาม kit) |
| `For_Tomorrow_Journey.h` | free | ATK% `12+4S` | กด ult → ตัวเอง DMG `15+3S` 1 เทิร์น |
| `Meshing_Cogs.h` | free | — | โจมตี หรือ ถูกโจมตี → energy `3+S` (ครั้งเดียวต่อเทิร์น) |
