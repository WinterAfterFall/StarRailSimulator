...
- User ท้วงถูกว่าการสรุปทางเข้ารันไว้ใน README อย่างเดียวไม่ใช่คู่มือรายไฟล์ตามรูปแบบงาน จึงเพิ่ม `Application.md`, `SettingFunction.md`, `Main.md`, `ManualBuilder.md`, `Setting.md` ที่ราก `instructor/` และลิงก์จาก README; ก่อนหน้านี้คำว่า "สำรวจจบ" หมายถึงอ่าน/สรุปภาพรวม แต่ยังไม่ได้ทำเอกสารรายไฟล์ จึงแก้ช่องว่างนี้แล้ว
- ไล่ `ManualBuilder.cpp` จบ: กำหนดทีม/อุปกรณ์/ศัตรูใน source, เปิด timing print แล้วทำลูป sim/reroll ใน `main()` แยกจาก interactive path; แก้แผนที่สถานะ README ที่ค้างจากช่วงเริ่มงาน เป็นสรุปปัจจุบันว่า `Class`/`Function` มีคู่มือไม่ว่างครบ แต่ `Data` ยังไม่สำรวจส่วนมาก
- สำรวจ `src/Setting.h`, `src/Enum/StatusEnum.h`, `src/StdInclude.h` ต่อจาก entry point: ค่า configuration กับ state กลางอยู่ใน `Setting.h`, enum โหมดอยู่ใน `StatusEnum.h`, ส่วน `StdInclude.h` รวม standard headers/priority macros/using; เพิ่มเส้นทางอ้างอิงใน README จุดต่อคือปรับตารางสถานะ README ที่ยังอ้างสถานะเก่า และไล่ `ManualBuilder.cpp` ที่เหลือหากต้องการครอบคลุมทางเข้าแบบ manual
- User ยืนยัน `EnemySelector()` ยังไม่ implement ตัวเลือกจำนวน/กำหนดศัตรูเอง จึง fix 2 ตัวชั่วคราว; ปรับคำอธิบายใน README และจดงานต่อใน `future-improvements.md` ไม่จัดเป็นบั๊ก
- ไล่ทางเข้า `Application.cpp`/`SettingFunction.h`/`Main.h` และตรวจ `ManualBuilder.cpp` ว่าเป็น entry point แยก บันทึกเส้นทางเลือกทีมและลูป run/wave ใน README; พบ `EnemySelector()` รับ `Tenemy` แต่ไม่ใช้ และทางเลือกตั้งค่าเองไม่สร้างศัตรู รอยืนยันเจตนาก่อนแก้
- ไล่ `src/Declaration/Function/Combat` ที่เหลือ (buff/debuff stack และ stat wrappers) กับ `Function/Event/Event.h` เทียบชื่อและ signature ที่สำคัญกับนิยาม; `allEventWhenAllyAction()` มีนิยามและถูกเรียกจาก Combat จริง ยังไม่พบ mismatch ใหม่ จุดต่อคือไฟล์เริ่มโปรแกรม `Application.cpp`/`SettingFunction.h`/`Main.h` และปรับตารางสถานะ README ที่ยังเก่า
- ตามคำขอ user จด cosmetic cleanup ที่พบระหว่างสำรวจ declaration ลง `future-improvements.md` (forward declaration/ฟังก์ชันประกาศซ้ำ และชื่อ `debuffEnemyTargetsyMark`) โดยยังไม่แก้โค้ด; ไล่ประกาศ `Function/Calculate` และ `Function/Combat` กลุ่ม action value, stats, HP, energy, target แล้ว ยังไม่พบ signature ที่ขัดกับนิยามที่เคยสำรวจ จุดต่อคือ Combat ส่วน buff/debuff/event และ entry point
- เริ่มสำรวจ `src/Declaration` นอกโครง mirror: `Library.h` แต่ละชั้นรวม header ตามหมวด, `Class/Class.h` เป็น forward declarations, ส่วน `Function/Setup` ประกาศ `Setup`/`Reset`/wave lifecycle และ reset/substats helpers ตรงกับนิยามที่ตรวจแล้ว; `Function/Combat/Debuff_Stats.h` และ `Dot.h` ประกาศ overload ตรงกับกลุ่ม wrapper ที่เพิ่งไล่ ปัจจุบันพบเพียงการประกาศซ้ำของ `printRoundResult()` ใน `Function/Print/Print.h` และ `CharUnit` ใน `Class/Class.h` ซึ่งยังไม่เห็นผลเสียขณะรัน; จุดต่อคือเทียบ declaration กลุ่ม Calculate/Combat ที่เหลือและ entry point
- กลับมาไล่ส่วนที่ยังไม่ละเอียดนอก `Data`: ตรวจ `SetCombat.h` เทียบลำดับเรียกจาก `Main.h` และขยายคู่มือให้ครอบคลุม `Setup()`, `Reset()`, `Start_game()`, `Start_wave()` และ `EndWave()`; จุดต่อคือสำรวจ `src/Declaration` กับ entry-point นอก `Defination`
- ไล่ wrapper ดีบัฟท้าย `Debuff_Stats.h` และ `Buff_Stats.h` ครบแล้ว: บันทึกการกระจายเป้าหมาย, Apply/Mark, การบวก stat เฉพาะเมื่อลงชื่อใหม่ และวงจรอายุบัฟในคู่มือ; ชื่อ `debuffEnemyTargetsyMark()` สะกดตามโค้ดและยังไม่มี caller ใน `Data` จุดต่อคือไฟล์ถัดไปใน `Function/Combat`
- User ยืนยันว่า `Print()` เป็น manual debug สำหรับตรวจความถูกต้องของการรัน บันทึกใน `Function/Print/Print.md`; ตรวจและบันทึก `printRoundResult()`, `printSummaryResult()` และ `PrintStats.h` ต่อแล้ว
- สำรวจ `Function/Event/Event.h` ต่อ พบและแก้ BUGS #21 หลัง user อนุมัติ: after-turn ลบ Break DoT แล้วอ่าน iterator ที่ชี้รายการถัดไป/ท้ายลิสต์; เก็บชนิดก่อนลบและเพิ่ม regression test
- หลังแก้ BUGS #21 สำรวจ `Function/Combat/EnemyCombat.h` ต่อ: เมธอดโจมตีสร้างแอ็กชันเข้าคิว, taunt มี dedup/ลบตามชื่อ; บันทึกในคู่มือแล้ว จุดต่อคือ `Function/Combat` ไฟล์ที่เอกสารยังว่างหรือสั้น โดยไม่ถามซ้ำสิ่งที่อธิบายแล้ว
- สำรวจ `Function/Combat/Dot.h` ครบกลุ่ม apply/mark/stack/remove และ wrapper แบบทุกศัตรู/เฉพาะเป้าแล้ว บันทึกเงื่อนไขการเพิ่มตัวนับชนิด DoT และการแยกจัดการอายุ debuff ไว้ใน `Dot.md`; จุดต่อคือ `Function/Combat` กลุ่ม buff/debuff
- เริ่มเทียบ `Function/Combat` กลุ่ม buff/debuff พบและแก้ BUGS #22 หลัง user ยืนยันว่า 0 stack ไม่ควรนับใน `Total_debuff`: `calDebuffStack()` ปรับจำนวนสถานะเฉพาะเมื่อ stack เปลี่ยนผ่าน 0 ↔ บวก มี regression test; จุดต่อคือดูการนับ `DotCount` ใน `dotSingleStack()` เมื่อขอเพิ่ม 0 หรือลด stack ที่ไม่มีอยู่
- User ยืนยันว่า DoT 0 stack ไม่ควรนับใน `DotCount`/ตัวนับรายชนิดเช่นกัน แก้ BUGS #23 ใน `dotSingleStack()` ทั้งสอง overload ให้ปรับตัวนับเฉพาะการเปลี่ยนผ่าน 0 ↔ บวก; คงลำดับอัปเดตตัวนับก่อน `AfterApplyDebuff` เพราะ Black Swan LC อ่าน `ShockCount` ที่ event นี้ มี regression test
- ไล่ทางถอน stack ต่อ: `debuffRemoveStack()` / `dotStackRemove()` ยังไม่มี caller ใน `Data`; `Black Swan.h` ใช้ `setStack("Arcana", stack/2)` หลัง DoT tick ซึ่งทำให้ 1 stack กลายเป็น 0 โดยไม่ปรับตัวนับ User ยืนยันว่าควรคง 1 stack จึงแก้ BUGS #24 ให้เหลือขั้นต่ำ 1 และเทียบพบว่า `Black SwanV1.h` ก็คง 1 stack มี regression test แล้ว
- สำรวจ `Function/Combat/Debuff_Stats.h` ต่อ พบ `weaknessApplyChoose()` ตั้ง `i = 1` ภายในลูป ทำให้ priority ของตัวละคร Path กลุ่มโจมตีทุกคนเท่ากัน; user แก้เองโดยย้าย `i` ไปก่อนลูปแล้ว ตรวจ diff บนดิสก์พบการแก้ตรงจุด บันทึกใน `Debuff_Stats.md`
- ไล่ `debuffApply()` / `debuffMark()` ต่อและบันทึกความต่างของจังหวะ Before/AfterApplyDebuff: Apply ยิงทุกครั้งที่พยายามลง, Mark ยิงเฉพาะเมื่อสร้างสถานะใหม่; การต่ออายุและการนับสถานะบันทึกใน `Debuff_Stats.md` จุดต่อคือ `weaknessApply()` และ wrapper ดีบัฟช่วงท้ายไฟล์
- ไล่ `weaknessApply()` / `weaknessApplyChoose()` ต่อแล้ว: เพิ่ม Weakness เฉพาะธาตุที่ยังไม่มี, เก็บวันหมดอายุที่ไกลกว่า, fallback ไปธาตุใน `Weakness_typeCountdown` เมื่อไม่มีตัวเลือกใหม่; จุดต่อคือ wrapper ดีบัฟช่วงท้าย `Debuff_Stats.h`
- User อนุมัติแก้ declaration mismatch ของ `createNewEnemy()` / `SetupEnemy()` จาก `string` เป็น `EnemyType`; เพิ่ม compile regression test ที่ล้มก่อนแก้และผ่านหลังแก้ พร้อมตรวจ syntax ของ `Application.cpp` ผ่าน ดู BUGS #20
- ไล่ `Function/Setup/SetEnemy.h`, `SetRelic.h` และ `Function/AdjustFunction` กลุ่ม `MainStatsRelic`, `RelicAdjust`, `CharCmd`, `EnemyCmd` แล้ว เติมคู่มือจากพฤติกรรมในโค้ด; จุดต่อคือกลุ่ม Print/Event และไฟล์ปรับ relic ที่เหลือ
- กลับมาสำรวจต่อจากจุดพักเดิมและเทียบ `src/Defination/Class/ActionData` กับคู่มือครบทุกไฟล์แล้ว
- เติมเอกสารส่วนที่ขาดโดยอิงจากโค้ด: constructors และ `addBuffSingleTarget()` แบบเลือกเป้าอัตโนมัติของ `AllyBuffAction`; การเพิ่ม action/damage type, `addEnemyToTargetList()` และคิว `AhaInstantBar` ของ `AllyAttackAction`
- ยืนยันว่า `Library.h` เป็น include aggregator ไม่มี logic เพิ่ม และปิดหมวด `Class/ActionData` ว่าสำรวจครบแล้ว
- จุดต่อ: ตรวจภาพรวม `Class` หลัง `CombatData` และ `Trigger` ที่ปิดไปแล้ว จากนั้นเริ่มหมวด `Function` ถัดจากไฟล์ที่มีคู่มือครบ
