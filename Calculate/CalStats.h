...
double Cal_Atk_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i=0,sz=data_->abilityType.size();i<sz;i++){
            Atk_percent_mtpr+= data_->source->Stats_type["Atk%"][data_->abilityType[i]];
            Atk_percent_mtpr+= target->Stats_type["Atk%"][data_->abilityType[i]];

            Flat_atk_mtpr += data_->source->Stats_type["Flat_Atk"][data_->abilityType[i]];
            Flat_atk_mtpr += target->Stats_type["Flat_Atk"][data_->abilityType[i]];
    }
...
double Cal_Hp_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i=0,sz=data_->abilityType.size();i<sz;i++){
            Hp_percent_mtpr+= data_->source->Stats_type["Hp%"][data_->abilityType[i]];
            Hp_percent_mtpr+= target->Stats_type["Hp%"][data_->abilityType[i]];

            Flat_hp_mtpr += data_->source->Stats_type["Flat_Hp"][data_->abilityType[i]];
            Flat_hp_mtpr += target->Stats_type["Flat_Hp"][data_->abilityType[i]];
    }
...
double Cal_Def_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i=0,sz=data_->abilityType.size();i<sz;i++){
            Def_percent_mtpr+= data_->source->Stats_type["Def%"][data_->abilityType[i]];
            Def_percent_mtpr+= target->Stats_type["Def%"][data_->abilityType[i]];

            Flat_def_mtpr += data_->source->Stats_type["Flat_Def"][data_->abilityType[i]];
            Flat_def_mtpr += target->Stats_type["Flat_Def"][data_->abilityType[i]];
    }
...
double Cal_Bonus_dmg_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i = 0, sz = data_->abilityType.size(); i < sz; i++){
        Bonus_dmg_mtpr += data_->Attacker->Stats_type[ST_DMG][data_->abilityType[i]] + data_->Attacker->Stats_each_element[ST_DMG][data_->Damage_element][data_->abilityType[i]];
        Bonus_dmg_mtpr += target->Stats_type[ST_DMG][data_->abilityType[i]] + target->Stats_each_element[ST_DMG][data_->Damage_element][data_->abilityType[i]];
    }
...
double Cal_Crit_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i=0,sz=data_->abilityType.size();i<sz;i++){
            Crit_rate_mtpr += data_->Attacker->Stats_type[ST_CR][data_->abilityType[i]] + target->Stats_type[ST_CR][data_->abilityType[i]];
            Crit_dam_mtpr += data_->Attacker->Stats_type[ST_CD][data_->abilityType[i]] + target->Stats_type[ST_CD][data_->abilityType[i]];
    }
...
double Cal_Def_shred_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i=0,sz=data_->abilityType.size();i<sz;i++){
            Def_shred_mtpr += data_->Attacker->Stats_type[ST_DEF_SHRED][data_->abilityType[i]] + target->Stats_type[ST_DEF_SHRED][data_->abilityType[i]];
    }
...
double Cal_Respen_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i = 0, sz = data_->abilityType.size(); i < sz; i++){
        Respen_mtpr += data_->Attacker->Stats_type["Respen"][data_->abilityType[i]] + data_->Attacker->Stats_each_element["Respem"][data_->Damage_element][data_->abilityType[i]];
        Respen_mtpr += target->Stats_type["Respen"][data_->abilityType[i]] + target->Stats_each_element["Respem"][data_->Damage_element][data_->abilityType[i]];
    }
...
double Cal_Vul_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i = 0, sz = data_->abilityType.size(); i < sz; i++){
        Vul_mtpr += data_->Attacker->Stats_type["Vul"][data_->abilityType[i]] + target->Stats_type["Vul"][data_->abilityType[i]];
    }
...
double Cal_BreakEffect_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i = 0, sz = data_->abilityType.size(); i < sz; i++){
        BreakEffect_mtpr += data_->Attacker->Stats_type[ST_BE][data_->abilityType[i]] + target->Stats_type[ST_BE][data_->abilityType[i]];
    }
...
double Cal_Mitigation_multiplier(shared_ptr<AllyActionData> &data_,Enemy *target){
    ...
    for(int i = 0, sz = data_->abilityType.size(); i < sz; i++){
        Mitigation_mtpr += data_->Attacker->Stats_type["Mitigation"][data_->abilityType[i]] + target->Stats_type["Mitigation"][data_->abilityType[i]];
    }
