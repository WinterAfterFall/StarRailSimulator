...
namespace Nihility_Lightcone{
    function<void(CharUnit *ptr)> Kafka_LC(int superimpose){
        return [=](CharUnit *ptr) {
            ...
            string Erode = ptr->getMemosprite()->getUnitName() + " Erode";
            ...
            AfterAttackActionList.push_back(TriggerByAllyAttackAction_Func(PRIORITY_IMMEDIATELY, [ptr,superimpose,Erode](shared_ptr<AllyAttackAction> &act) {
                if(act->isSameUnitName(ptr->getMemosprite())){
                    ptr->getMemosprite()->buffStackSingle({{Stats::SPD_P,AType::None,4.0 + 0.8*superimpose}},1,3,"Kafka LC");
                    for(auto &each : act->targetList ){
                        if(!each->getDebuff(Erode))each->dotSingleApply({DotType::Shock},ptr->getMemosprite(),Erode,1);
                    }
                }
            }));
            ...
            Dot_List.push_back(TriggerDot_Func(PRIORITY_IMMEDIATELY, [ptr,superimpose,Erode](Enemy* target, double Dot_ratio,DotType Dot_type) {
                ...
                if (target->getDebuff(Erode)){
                    ...
                    make_shared<AllyAttackAction>(AType::Shock,ptr->getMemosprite(),TraceType::Single,Erode);
