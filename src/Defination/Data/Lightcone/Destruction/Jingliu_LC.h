        ...
        return [=](Ally *ptr) {
            ...
            Enemy_hit_List.push_back(TriggerByEnemyHit(PRIORITY_ACTTACK,[ptr,superimpose](Enemy *Attacker,vector<SubUnit*> target){
                for(SubUnit* e : target){
                    ptr->getSubUnit()->buffStackSingle({{Stats::DMG,AType::None,11.5 +2.5*superimpose}},1,3,"Jingliu_LC");
                }
                if(ptr->getSubUnit()->getStack("Jingliu_LC")>=3){
                    if(ptr->getSubUnit()->isHaveToAddBuff("Jingliu_LC Def Shred"))
                        ptr->getSubUnit()->buffSingle({{Stats::DEF_SHRED,AType::None,10.0 +2*superimpose}});
                }
            }));
            ...
            AfterAttackActionList.push_back(TriggerByAllyAttackAction_Func(PRIORITY_IMMEDIATELY,[ptr,superimpose](shared_ptr<AllyAttackAction> &act){
                ...
                ptr->getSubUnit()->buffResetStack({{Stats::DMG,AType::None,11.5 +2.5*superimpose}},"Jingliu_LC");
