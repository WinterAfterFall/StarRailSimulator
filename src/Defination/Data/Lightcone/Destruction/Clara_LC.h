    ...
    function<void(Ally *ptr)> Blade_LC(int superimpose){
        return [=](Ally *ptr) {
            ...
            Reset_List.push_back(TriggerByYourSelf_Func(PRIORITY_IMMEDIATELY,[ptr,superimpose](){
                ptr->Sub_Unit_ptr[0]->Stats_type[ST_ATK_P][AT_NONE]+=20 + (4*superimpose);
            }));
            Enemy_hit_List.push_back(TriggerByEnemyHit(PRIORITY_ACTTACK,[ptr,superimpose](Enemy *Attacker,vector<SubUnit*> target){
                for(SubUnit* e : target){
                    if(e->isSameUnit(ptr->Sub_Unit_ptr[0].get())){
                        ...
                        e->buffSingle({{ST_DMG,AT_NONE,(20.0 + 4*superimpose)}},"Clara_LC",1);
                        return;
                    }
                }
            }));
            After_turn_List.push_back(TriggerByYourSelf_Func(PRIORITY_IMMEDIATELY, [ptr,superimpose]() {
                if(ptr->getSubUnit()->isBuffEnd("Clara_LC")){
                    ptr->getSubUnit()->buffSingle({{ST_DMG,AT_NONE,-(20.0 + 4*superimpose)}});
                }
            }));
