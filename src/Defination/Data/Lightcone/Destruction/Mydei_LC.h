    ...
    function<void(Ally *ptr)> Mydei_LC(int superimpose){
        return [=](Ally *ptr) {
            ...
            Reset_List.push_back(TriggerByYourSelf_Func(PRIORITY_IMMEDIATELY, [ptr,superimpose]() {
                ptr->Sub_Unit_ptr[0]->Stats_type[ST_HP_P][AType::None] += 15 + 3*superimpose;
                ptr->Sub_Unit_ptr[0]->Stats_type[ST_HEALING_OUT][AType::None] += 15 + 5 * superimpose;
            }));
            ...
            BeforeAttackAction_List.push_back(TriggerByAllyAttackAction_Func(PRIORITY_IMMEDIATELY, [ptr,superimpose](shared_ptr<AllyAttackAction> &act) {
                ...
                if (act->isSameAttack(AType::SKILL)||act->isSameAttack(AType::Ult)) {
                    ...
                    ptr->getSubUnit()->buffSingle({{ST_DMG, AType::None, (25.0 + 5 * superimpose)}});
            ...
            AfterAttackActionList.push_back(TriggerByAllyAttackAction_Func(PRIORITY_IMMEDIATELY, [ptr,superimpose](shared_ptr<AllyAttackAction> &act) {
                if (!act->Attacker->isSameUnit(ptr->Sub_Unit_ptr[0].get())) return;
                ptr->getSubUnit()->buffSingle({{ST_DMG, AType::None, -(25 + 5 * superimpose) * ptr->Sub_Unit_ptr[0]->Buff_note["Mydei_LC_Mark"]}});
